...
int altura_Disciplina(Arv_disc *R) {
    int valor; 
    if (R == NULL) {
...
void atualizarAltura_Disciplina(Arv_disc *R) {
    int valor, esq, dir; 
    if (R != NULL) {
        esq = altura_Disciplina(R->esq); 
        dir = altura_Disciplina(R->dir); 
        if(esq > dir){
            valor = 1 + esq; 
        }else{
            valor = 1 + dir; 
...
void rotacaoDir_Disc(Arv_disc **R) {
    ...
    atualizarAltura_Disciplina(*R);
    atualizarAltura_Disciplina(novo_R);
...
int fatorBalanceamento_Disc(Arv_disc *R) {
    int valor, esq, dir; 
    if (R == NULL) {
        valor = 0;
    }else{
...
void balanceamento_ArvAVL_Disciplina(Arv_disc **R){
    int fb; 
    fb = fatorBalanceamento_Disc(*R);
    if(fb == -2){
        if(fatorBalanceamento_Disc((*R)->dir) > 0){
            rotacaoDir_Disc(&((*R)->dir)); 
        }
        rotacaoEsq_Disc(R); 
    }else if(fb == 2){
        if(fatorBalanceamento_Disc((*R)->esq) < 0){
            rotacaoEsq_Disc(&((*R)->esq));
        }
        rotacaoDir_Disc(R); 
    }
}
...
int inserirArvBB_Disc(Arv_disc **disciplinas, Arv_disc *novo) {
    ...
    atualizarAltura_Disciplina(*disciplinas); //Função que serve


    balanceamento_ArvAVL_Disciplina(disciplinas);
...
int ArvBB_Disc_Remover(Arv_disc **R, int codigo_disciplina){
    ...
    if (*R != NULL) {
        atualizarAltura_Disciplina(*R);  // Atualiza a altura do nó
        balanceamento_ArvAVL_Disciplina(R);  // Balanceia a árvore após a remoção
    }
